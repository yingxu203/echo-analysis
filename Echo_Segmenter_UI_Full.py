...
def runButtonClicked():
    ...
    # Use the same Python interpreter/environment this GUI was launched
    # with, rather than relying on `conda activate` inside a subshell
    # (fragile, and the original 'tf-latest' env name no longer exists on
    # this machine anyway).
    ...
    comm2 = write_command(studyDir.get(),runMode,'M-Mode',verboseString, python_exe)

    # If B-Mode data present, add command to run B-Mode analysis.
    comm3 = write_command(studyDir.get(),runMode,'B-Mode',verboseString, python_exe)

    # Combine into 1 command string (macOS/Linux shell: chain with &&, not
    # the Windows cmd.exe '&' the original script used)
    commandString = ' && '.join(c for c in [comm2, comm3] if c)
