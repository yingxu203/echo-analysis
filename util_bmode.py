...
def findLongAxis(mask):
    ...
    # No LV contour found at all (e.g. empty prediction on this frame)
    if len(sort) < 1:
        return None, None

    # Take the largest contour
    cont = np.squeeze(sort[0])
    # Degenerate single-point contour: no meaningful long axis to measure
    if cont.ndim == 1:
        return None, None
...
def computeMetrics(labels, systoles, diastoles, res_x, res_y):
    ...
    for sys_label in sys_labels:
        pt1, pt2 = findLongAxis(sys_label)
        if pt1 is None:
            # no LV contour found on this frame (empty/degenerate prediction) - skip it
            continue
    ...
    sys_area_mean = np.mean(np.array(sys_areas)) if sys_areas else np.nan
    sys_volume_mean = np.mean(np.array(sys_volumes)) if sys_volumes else np.nan
    ...
    dia_area_mean = np.mean(np.array(dia_areas)) if dia_areas else np.nan
    dia_volume_mean = np.mean(np.array(dia_volumes)) if dia_volumes else np.nan
...
def postprocess_masks(masks, contappr=False):
    ...
    for i in range(masks.shape[0]):
        ...
        # Center of Contour
        cont_pts = np.squeeze(cont)
        if cont_pts.ndim == 1:
            # degenerate single-point contour (e.g. near-empty prediction) -
            # np.squeeze collapses it to shape (2,), so there's nothing to average
            x_center, y_center = cont_pts.astype(np.uint32)
        else:
            x_center, y_center = (np.mean(cont_pts, axis=0)).astype(np.uint32)
