...
if __name__ == '__main__':
    ...
    # load trained model
    WEIGHTS_PATH = './model_weights/weights_MMode_finetuned_v3.h5'
    model = get_unet()
    model.load_weights(WEIGHTS_PATH)

    # Create a new, clearly-labeled output directory for this run instead of
    # overwriting the previous run's output.csv / output_images.pdf
    weights_label = os.path.splitext(os.path.basename(WEIGHTS_PATH))[0]
    run_timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = os.path.join(input_dir, 'output', f'run_{run_timestamp}_{weights_label}')
    ...
    for file in files:
        ...
        try:
            ...
            if len(np.shape(img_raw)) == 4:
                ...
                raise ValueError(f"unsupported image shape {np.shape(img_raw)}")
            images = []
            ...
            # Select frame to use
            # Score each frame by "coverage": the fraction of columns where all 4
            # anatomical layers (background, LVAW, LVID, LVPW) are clearly present.
            # A frame with high coverage represents a clean, complete cardiac cycle
            # capture - this is a better indicator of a "good" frame than raw model
            # confidence alone, which can be high even for a poorly-timed frame.
            coverage_scores = []
            for z in range(np.shape(label)[0]):
                frame_label = label[z, :, :]
            ...
            # Consider any frame within 2% of the best coverage as "tied"
            candidate_idxs = [i for i, c in enumerate(coverage_scores) if c >= max_coverage - 0.02]
            # Among those, pick the one with highest model confidence
            ...
            if args.verbose:
                ...
                # Determine line color based on filename
                fname_upper = file.upper()
                if any(tag in fname_upper for tag in ['WT', 'WF', 'WM']):
                    line_color = (0/255, 0/255, 255/255)   # #0000FF
                elif any(tag in fname_upper for tag in ['OX', 'OF', 'OM']):
                    line_color = (255/255, 44/255, 44/255) # #FF2C2C
                else:
                    line_color = (1, 1, 0)  # default yellow, fallback

                ab = mark_boundaries(a, b, color=line_color)
                ab = rescale_intensity(ab, out_range=(0, 255))
                ab = ab.astype('uint8')
                segs.append(ab)
        except Exception as e:
            print(f"WARNING: failed to process {file}: {e}")
            LVAWs_s.append(np.nan)
            LVAWs_d.append(np.nan)
            LVIDs_s.append(np.nan)
            LVIDs_d.append(np.nan)
            LVPWs_s.append(np.nan)
            LVPWs_d.append(np.nan)
            FSs.append(np.nan)
            LV_Masses.append(np.nan)
            LV_Mass_Cors.append(np.nan)
            Heart_Rates.append(np.nan)
            frames_used.append(np.nan)
            if args.verbose:
