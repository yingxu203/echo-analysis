...
if __name__ == '__main__':
    ...
    # load trained model
    WEIGHTS_PATH = './model_weights/weights_BMode_finetuned.h5'
    model = get_unet()
        ...
        try:
            ...
            if args.verbose:
                ...
                if any(tag in fname_upper for tag in ['WT', 'WF', 'WM']):
                    line_color = (0/255, 0/255, 255/255)   # #0000FF blue
                elif any(tag in fname_upper for tag in ['OX', 'OF', 'OM']):
                    line_color = (255/255, 44/255, 44/255) # #FF2C2C red
                else:
            ...
            sys_areas.append(np.nan)
            dia_areas.append(np.nan)
            sys_volumes.append(np.nan)
            dia_volumes.append(np.nan)
            ejection_fractions.append(np.nan)
            if args.verbose:
                blank = np.zeros((256, 256, 3), dtype='uint8')
                sys_segs.append(blank)
                dia_segs.append(blank)
