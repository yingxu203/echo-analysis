...
def crop_frame(img):
    ...
    if img.ndim == 3 and img.shape[2] >= 3:
        thresh_input = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    elif img.ndim == 3 and img.shape[2] == 1:
        thresh_input = img[:, :, 0]
    else:
        thresh_input = img
    thresh_input = thresh_input.astype(np.uint8)
    ret, threshed_img = cv2.threshold(thresh_input, 1, 255, cv2.THRESH_BINARY)
    ...
    if cropped.ndim == 3 and cropped.shape[2] >= 3:
        img_out = rgb2gray(cropped)
    elif cropped.ndim == 3 and cropped.shape[2] == 1:
        img_out = cropped[:, :, 0]
    else:
        img_out = cropped
    return img_out
